<?php
$jasperserver_url = "http://localhost:8085/jasperserver/services/";
...
function ws_get($uri, $args = array(), $user = array()) {
//	global $_SESSION;

	$connection_params = array("user" => $user["username"], "pass" => $user["password"]);
	$info = new SOAP_client($args["webservices_uri"], false, false, $connection_params);
	...
	$params = array("request" => $op_xml);
	$response = $info->call("get", $params, array('namespace' => $args["namespace"]));

	return $response;
}

function ws_runReport($uri, $report_params, $output_params, &$attachments, $user = array()) {
//	global $_SESSION;
	$max_execution_time = 120; // 2 mins.

	$connection_params = array("user" => $user["username"], "pass" => $user["password"], "timeout" => $max_execution_time);
	$info = new SOAP_client($user["webservices_uri"], false, false, $connection_params);
